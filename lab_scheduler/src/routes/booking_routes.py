    ...
    try: # Wrap slot processing in try/except
        for slot_input in slots_data:
            ...
            if period not in ["Manhã", "Tarde"]:
                ...
                return jsonify({"error": f"Período inválido '{period}'. Use 'Manhã' ou 'Tarde'"}), 400 # Corrected f-string
            try:
                ...
                return jsonify({"error": f"Formato de data inválido '{booking_date_str}'. Use YYYY-MM-DD"}), 400 # Corrected f-string
        ...
        for booking_date_obj, count_for_this_request in daily_new_bookings_count.items():
            ...
            if (existing_bookings_on_day + count_for_this_request) > MAX_BOOKINGS_PER_DAY:
                ...
                return jsonify({"error": f"Limite de {MAX_BOOKINGS_PER_DAY} agendamentos/dia para '{user_name}' excedido em {booking_date_obj.strftime('%Y-%m-%d')}."}), 409 # Corrected f-string
        ...
        for slot in processed_slots:
            if slot['room_name'].startswith("Geral "): # Corrected f-string access
                geral_slots_in_request_by_day[slot['booking_date_obj']].append( # Corrected f-string access
                    {"room_id": slot['room_id'], "period": slot['period']} # Corrected f-string access
                )
        ...
        for booking_date_obj, requested_geral_slots in geral_slots_in_request_by_day.items():
            ...
            for req_slot in requested_geral_slots:
                 if (req_slot['room_id'], req_slot['period']) not in existing_tuples: # Corrected f-string access
                     combined_geral_slots.append(req_slot)
            geral_periods_booked = {slot['period'] for slot in combined_geral_slots} # Corrected f-string access
            geral_rooms_booked_ids = {slot['room_id'] for slot in combined_geral_slots} # Corrected f-string access
            ...
            if num_geral_periods > 2:
                ...
                return jsonify({"error": f"Não é possível agendar mais de dois períodos ('Manhã' e 'Tarde') em salas 'Geral' no mesmo dia ({booking_date_obj.strftime('%Y-%m-%d')})."}), 409 # Corrected f-string
            if num_geral_periods == 2 and num_geral_rooms > 2:
                 ...
                 return jsonify({"error": f"Não é possível agendar mais de duas salas 'Geral' diferentes no mesmo dia ({booking_date_obj.strftime('%Y-%m-%d')})."}), 409 # Corrected f-string
        ...
        for slot in processed_slots:
            if check_booking_conflict(slot['room_id'], slot['booking_date_obj'], slot['period']): # Corrected f-string access
                current_app.logger.info(f"Booking conflict found: Room {slot['room_id']}, Date {slot['booking_date_str']}, Period {slot['period']}") # Corrected f-string access
                return jsonify({"error": f"Sala '{slot['room_name']}' já reservada para '{slot['period']}' em {slot['booking_date_str']}."}), 409 # Corrected f-string access
        ...
        for slot in processed_slots:
            new_booking = Booking(
                ...
                room_id=slot['room_id'], booking_date=slot['booking_date_obj'], period=slot['period'] # Corrected f-string access
            )
            db.session.add(new_booking)
            newly_created_bookings_details_for_email.append({
                "room_name": slot['room_name'], "booking_date": slot['booking_date_str'], "period": slot['period'] # Corrected f-string access
            })
        db.session.commit()
...
def get_booking_status():
    ...
    try:
        ...
        thursday_current_week = start_of_current_week + timedelta(days=RELEASE_WEEKDAY)
        release_datetime_for_next_week = datetime.combine(thursday_current_week, RELEASE_TIME)
        # Adjust release time to be on Friday UTC if RELEASE_TIME is past midnight UTC
        if RELEASE_TIME < time(0,0,0) or (RELEASE_TIME > time(0,0,0) and RELEASE_TIME < time(3,0,0)):
             release_datetime_for_next_week += timedelta(days=1)
        current_app.logger.debug(f"Next week release UTC: {release_datetime_for_next_week}")
