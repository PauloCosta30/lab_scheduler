...
document.addEventListener("DOMContentLoaded", () => {
    ...
    function checkIfBookingAllowed(dateStr) {
        ...
        try {
            ...
            currentWeekFriday18h.setUTCHours(18, 0, 0, 0); // 18:00:00.000 UTC

            const nextWeekWednesday2359 = new Date(nextWeekMonday.valueOf());
            nextWeekWednesday2359.setUTCDate(nextWeekMonday.getUTCDate() + 2); // Segunda + 2 dias = Quarta
            nextWeekWednesday2359.setUTCHours(23, 59, 59, 999); // 23:59:59.999 UTC

            const nowUTC = new Date(); // Data e hora atual em UTC

            // Lógica para a semana atual
            if (slotDate >= currentWeekMonday && slotDate < nextWeekMonday) {
                if (nowUTC <= currentWeekWednesday) {
                    ...
                    console.log(`Semana atual (antes da quarta 23:59) - ${dateStr}: true`);
                    return true;
                } else {
                    // Se já passou da quarta-feira 23:59 da semana atual, a semana atual está fechada
                    console.log(`Semana atual (depois da quarta 23:59) - ${dateStr}: false (prazo encerrado)`);
                    return false;
                }
            } 
            // Lógica para a próxima semana
            else if (slotDate >= nextWeekMonday && slotDate < new Date(nextWeekMonday.getTime() + 7 * 24 * 60 * 60 * 1000)) {
                if (nowUTC >= currentWeekFriday18h && nowUTC <= nextWeekWednesday2359) {
                    // Se já é sexta-feira 18h da semana atual e ainda não passou da quarta-feira 23:59 da próxima semana
                    console.log(`Próxima semana (aberta) - ${dateStr}: true`);
                    return true;
                } else {
                    // Fora do período de agendamento da próxima semana
                    console.log(`Próxima semana (fechada) - ${dateStr}: false`);
                    return false;
                }
            } 
            // Para datas além da próxima semana, elas devem ser consideradas fechadas por padrão
            else {
