...
document.addEventListener("DOMContentLoaded", () => {
    ...
    async function loadScheduleData(selectedDateStr) {
        ...
        if (selectedDateStr) {
            // If a date is selected, load that specific week
    ...
    // Renders the schedule table for 7 days (Mon-Sun)
    function renderScheduleTable(bookings, roomsData, weekStartDateObj) {
        ...
        const days = ["Segunda", "Terça", "Quarta", "Quinta", "Sexta", "Sábado", "Domingo"]; // 7 days
    ...
    function checkBookingWindowFrontend(slotDateUTC) {
        ...
        const currentWeekEnd = parseDateStrToUTC(currentBookingStatus.current_week_end); // Sunday
        const nextWeekEnd = parseDateStrToUTC(currentBookingStatus.next_week_end); // Sunday
        ...
        // Check if the slot date falls within the current week (Mon-Sun)
        ...
        // Check if the slot date falls within the next week (Mon-Sun)
