...
document.addEventListener("DOMContentLoaded", () => {
    ...
    // FUNÇÃO CORRIGIDA - Verificar se a data está no passado
    function isDateInPast(dateStr) {
        const todayUTC = getTodayUTC();
        const targetDate = parseDateStrToUTC(dateStr);
        
        console.log(`Verificando se ${dateStr} está no passado:`);
        console.log(`  Hoje UTC: ${todayUTC.toISOString().split("T")[0]}`);
        console.log(`  Data alvo: ${targetDate.toISOString().split("T")[0]}`);
        console.log(`  É passado? ${targetDate < todayUTC}`);
    ...
    // FUNÇÃO COMPLETAMENTE REESCRITA - Verificação precisa dos horários de agendamento
    function checkIfBookingAllowed(dateStr) {
        console.log(`=== VERIFICANDO AGENDAMENTO PARA ${dateStr} ===`);
        
        try {
            // PRIMEIRO: Verificar se a data não está no passado
            if (isDateInPast(dateStr)) {
                console.log(`❌ Data ${dateStr} está no passado`);
                return {
                    allowed: false,
                    displayText: "Passado",
                    cssClass: "past",
                    title: "Esta data já passou"
                };
            }
    ...
    function handleSlotClick(event) {
        ...
        // VALIDAÇÃO DUPLA: Verificar se não está no passado E se o agendamento é permitido
        if (isDateInPast(slotDateStr)) {
            console.log(`Data ${slotDateStr} está no passado`);
            showScheduleMessage("Não é possível agendar datas no passado.", "error");
            return;
        }
